...
class DataHandler(object):
    ...
    @classmethod
    def load(klass, ifile = 'data/inputs.csv', tfile = 'data/targets.csv'):
        data, target = klass._load_files(ifile, tfile)
        assert not data.isnull().values.any(), 'Your data has null entries, clean it'
        data = klass._time_data(data, target)
        return data

    @classmethod
    def load_train_test(klass, ifile = 'data/inputs.csv', tfile = 'data/targets.csv', fraction = 0.25):
        ...
        assert train.timeStamp.max() < test.timeStamp.min(), \
            "Problems with your data, make sure that you are trying to predict future"
